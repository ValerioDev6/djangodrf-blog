...
class HeadingModelTest(TestCase):
    def setUp(self):
        ...
        self.heading = Heading.objects.create(
            post=self.post, title="Heading 1", slug="heading-1", level=1, order=1
        )
...
class PostDetailViewTest(TestCase):
    def setUp(self):
        ...
        self.category = Category.objects.create(
            name="Detail Category", slug="detail-category"
        )
        self.post = Post.objects.create(
            title="Detail Post",
    ...
    def test_get_post_detail_success(self, mock_increment_views):
        ...
        mock_increment_views.assert_called_once_with(
            self.post.slug, "127.0.0.1"
        )  # IP predeterminada en tests
...
class PostHeadingsViewTest(TestCase):
    def setUp(self):
        ...
        self.heading2 = Heading.objects.create(
            post=self.post, title="Heading 2", slug="heading-2", level=2, order=2
        )
...
class IncrementPostClickViewTest(TestCase):
    def setUp(self):
        ...
        self.category = Category.objects.create(
            name="Analytics Category", slug="analytics-category"
        )
        self.post = Post.objects.create(
    ...
    def test_increment_post_click_success(self):
        ...
        """
        url = reverse("post-increment-clicks")

        response = self.client.post(
            url, {"slug": self.post.slug}, HTTP_API_KEY=self.api_key, format="json"
        )
